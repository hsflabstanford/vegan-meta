...
\toprule
Study Characteristic & N (Studies) & N (Estimates) & SMD & 95\% CIs & \shortstack{Subset \\ $p$ value} & \shortstack{Moderator \\ $p$ value}\\
\midrule
\addlinespace[0.3em]
...
\bottomrule
\multicolumn{7}{l}{\textsuperscript{} Moderation analyses by differences in outcomes, population, region, decade of publication, and delivery method. The first p value column tests the hypothesis that the}\\
\multicolumn{7}{l}{subset of studies with a given characteristic is significantly different than an SMD of zero. The second compares effects within a given group, with the top category set}\\
\multicolumn{7}{l}{to reference.}\\
\end{tabular}
\end{table}
