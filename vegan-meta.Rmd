...
Finally, `r nrow(hybrid_norm_approaches)` deploy what we consider to be norms-based nudges in unobtrusive ways in dining halls, e.g., sign that said "In a taste test we did at the [name of cafe], 95% of people said that the veggie burger tasted good or very good!" [@piester2020, p. 5], and a similar message in a chain of restaurants [@coker2022].  The question of whether these interventions are nudges is debatable. On the one hand, they seek to exploit a widespread heuristic of following group norms, and they do so by raising the salience of certain items in an unobtrusive way. On the other, They are plainly attempts to _change_ consumer preferences by providing factual information rather than taking preferences as a given and trying to help them achieve them. However, with these two interventions, the suggested change is implicit and can easily be ignored, wbhich militates towards a "libertarian paternalistic" perspective on choice architecture. A further relevant distinction comes from @brachem2019, who argue that while descriptive norms can arguably be nudges, injunctive norms "work at least partly...through the implication of social sanctions" (p.9), which are effectively incentives. Therefore, we coded any norm-based intervention that was cheap, unobtrusive, and descriptive as a nudge as well. For example, a norm-based message on a menu is a nudge, but a survey with a norm message taken on the line to a dining hall is not due to being obtrusive.

### 3.2.2 Nudges
```{r nudge_theory, eval=F, include=F, echo=F}
nudge_studies <- dat |> filter(str_detect(theory, "nudge"))


length(unique(nudge_studies$doi_or_url))
```
