...
text_based <- nudge_studies |> filter(str_detect(secondary_theory, "text"))
text_based_country <- noquote(unique(text_based$country))
num_text_studies <- length(unique(text_based$unique_paper_id))
```
...
By these standards, `r nrow(nudge_studies)` intervention in our database are nudges printed in `r length(unique(nudge_studies$doi_or_url))` papers. A full `r hybrid_nudge_approach_number` of these studies are hybrid approaches. `r nrow(norms_nudge_approaches)` of these, as previously discussed, are norms-based nudges (though one also has an environmental message: @piester2020). The remaining `r nrow(nudge_studies) - nrow(norms_nudge_approaches)` combine nudges with information about the health or environmental benefits of reducing MAP consumption. Two interventions in @piester2020 placed a message about the environmental benefits of reducing MAP consumption in a university canteen with no social norm message. The remaining `r nrow(text_based)` interventions are text-based nudges, coming from `r `num_text_studies` papers all taking place in `r text_based_country` and all with Valentina Carfora as lead author. [describe these papers a bit]

`r pure_nudge_count` study is a pure nudge, @andersson2021
