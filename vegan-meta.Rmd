...
advocacy_results <- dat |> mutate(advocacy_materials = if_else(advocacy_org == "N", T, F)) |> 
  split(~advocacy_materials) |> map(count_and_robust); advocacy_results 
```

As suggested earlier by our tests for publication bias, evaluations of materials provided by animal advocacy organizations are not effective at reducing MAP consumption. `r advocacy_results$'TRUE'['N_unique']` studies were either conducted by animal advocacy organizations or used their materials. Table XXX displays the effect sizes associated with these studies next to those from everything else.

[table XXX about here]

| Materials | N (studies)                                | DELTA (se)                                                                                                                                                                                                                                                                                              |
|-----------|-----------|---------------------------------------------------|
| Advocacy  | `r advocacy_results$'FALSE'[['N_unique']]` | `r formatC(advocacy_results$'FALSE'[['Delta']], format = 'f', digits = 2)` (`r advocacy_results$'FALSE'[['se']]`)**`r ifelse(advocacy_results$'FALSE'[['pval']] < 0.05, "*", ifelse(advocacy_results$'FALSE'[['pval']] < 0.01, "**", ifelse(advocacy_results$'FALSE'[['pval']] < 0.001, "***", "")))` |
| Academic  | `r advocacy_results$'TRUE'[['N_unique']]`  | `r formatC(advocacy_results$'TRUE'[['Delta']], format = 'f', digits = 2)` (`r advocacy_results$'TRUE'[['se']]`)***`r ifelse(advocacy_results$'TRUE'[['pval']] < 0.05, "*", ifelse(advocacy_results$'TRUE'[['pval']] < 0.01, "**", ifelse(advocacy_results$'TRUE'[['pval']] < 0.001, "***", "")))`    |
