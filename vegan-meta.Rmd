...
abstract: |
  This paper meta-analyzes interventions intended to reduce consumption of meat and animal products (MAP), focusing on the most rigorous, policy-ready studies. Extant efforts generally embody one of three theoretical perspectives: appeals to social norms, nudges to make MAP less salient, and persuasion-based approaches that attempt to change attitudes towards MAP itself on health, environmental, and animal welfare grounds. We find that nudges produce the largest results overall, though the the effects are substantively small. Some environmental and health appeals reduce MAP consumption in some rarefied settings, while appeals to animal welfare, online studies, video treatments, and leafletting studies have no apparent overall effect. By contrast, a variety of approaches are effective at reducing consumption of red and processed meat, but the extent to which this is driven by substitutions to other categories of MAP is unclear...[something about biases?] We conclude with concrete, shovel-ready suggestions for researchers and policymakers. 
...
```{r libraries_and_function, include=F, echo=F}
# libraries
...
Worldwide consumption of meat and animal products (MAP) is a large and stubborn contributor to numerous policy challenges. Animal agriculture is a major driver of climate change [@scarborough2023; @koneswaran2008; @goodland2009] as well as more localized environmental and public health concerns [@horrigan2002; @slingenbergh2004; @graham2008; @greger2010]. Excess MAP consumption is a leading cause of premature deaths [@willett2019; @landry2023]; finally, how farmed animals live and die is increasingly recognized as a policy matter in its own right [@yeates2011; @webster2001; @kuruc2023].

Policymakers have many tools available for reducing MAP consumption. They can can ban certain foods [@caro2009] or practices [@bursey2018]; campaign for vegetarianism [@trewern2022]; raise taxes on MAP with high externalities [@springmann2018]; or change eating environments to make MAP alternatives more salient or appealing [@guthrie2015; @bianchi2018restructuring]. However, any attempt to limit supply rather than reducing demand is at high risk of backsliding through political correction [@michielsen2022]. It is essential, therefore, to assess which theoretical approaches most effectively and durably reduce demand for MAP. This paper approaches that question with a meta-analysis of the most policy-relevant MAP reduction literature.
...
Scholars in many disciplines have studied MAP reduction, including social psychology [@rosenfeld2018; @dhont2019], economics [@lusk2009], choice architecture [@bianchi2018restructuring; @mertens2022], and environmental studies [@costello2016]. Among the studies that meet our inclusion criteria, three approaches prevail: **norms-based approaches** that make MAP seem socially undesirable, **nudges** that make MAP alternatives more salient, and **persuasion-based approaches** that attempt to change ideas about the desirability of MAP itself. Persuasion approaches decompose into health, environmental, and animal welfare appeals. Which approach among these is most effective at changing behavior has broad implications for both this particular issue and policy in general.

Our main quantitative finding is that while individual studies have shown encouraging results, none of the studied approaches consistently reduces MAP reduction. While we find some evidence that some approaches have succeeded in reducing consumption of red and processed meat, there is an open question about how much of that change is driven by substitution to other categories of MAP. We find strictly null results for appeals to animal welfare, studies conducted online, video treatments, and leafletting studies, and a slight backlash to evaluations of materials produced by animal advocacy organizations.

For nudges in particular, we note a general lack of engagement with the possibility of compensatory/backlash effects down the line. We think of this as the "regression to the meat" problem: that someone who is nudged into eating less MAP at one meal may perceive a moral license [@merritt2010; @blanken2015] to eat more MAP later. If we limit our analysis to studies with deal convincingly with both threats to inference, we conclude that appeals to the environment and health reduce reduce MAP consumption at a single elite American college. For this literature to truly become shovel-ready for policymakers, it urgently needs extension and replication, along with careful attention to measurement validity.

Our paper builds on two literatures. The first is systematic reviews of attempts to reduce MAP consumption, of which there have been 22 by our count, with two others that we know of forthcoming. (See appendix A for a complete overview). Our paper makes four contributions to this already rich set of papers. First, our database is current as of December 2023, which has significance in an emerging literature whose most credible estimates and best designs tend to be in recent publications. Second, our review is quantitative, while most previous reviews are narrative or systematic reviews. Third, among papers with a meta-analytic component, ours is (so far) unique for being theoretically comprehensive rather than focusing on the effects of a single conceptual approach. Fourth, this paper is the first review, to our knowledg,e to set strict inclusion criteria that attempt to identify the most rigorous, policy-ready research.[^1]

[^1]: Some prior reviews also err very strongly on the side of study inclusion, and thus meta-analyze designs with serious threats to internal validity. For example, prior reviews included studies that use differential screening procedures for treatment and control [@rees2018] or that alter the treatment after assignment based on subjects' prior diets [@morren2021]; these procedures violate the 'all else equal in expectation' condition of a randomized controlled trial. As @simonsohn2022 put it, combining "studies that lack internal validity or external validity, which are obtained using incorrect statistical techniques, or studies where results seem to arise from methodological artefacts" with high-quality studies creates averages that are "virtually guaranteed to lack a meaningful interpretation" (p. 551).

Second, our paper contributes to a growing literature of meta-analyses, reviews, and "megastudies" [@doell2023; @milkman2021; @milkman2021megastudy] that attempt to compare the efficacy of different approaches to solving social problems. In social psychology, @paluck2021 and @porat2024 apply meta-analytic methods to testing the efficacy of different theories at reducing prejudice and sexual violence, respectively, while @vlasceanu2024 test 11 theoretically diverse interventions aimed at four "climate mitigation outcomes" (p. 1). In a related vein, @bergquist2023 meta-analyze meta-analyses on climate mitigation behaviors and find that social comparison and financial incentives were generally effective, while information and feedback generally were not. We also build on prior reviews that holistically assess the efficacy of choice architecture approaches versus, e.g., taxes [@list2023] or subsidies [@campos2021]. Finally, @bergman2024 compare information-based approaches to "short-term financial assistance, customized assistance during the housing search process, and connections to landlords" (p. 1) to encourage families to move to high-opportunity areas. Our paper's approach is similar to all of these but distinct, to the best of our knowledge, for the stringency and policy focus of our meta-analytic inclusion criteria.
...
Section 5 presents our quantitative results. We provide a mixture of pooled averages, tests for publication biases, comparisons between different approaches, and the effects of moderators such as the presence of a pre-analysis plan or a longer delay between treatment and measurement. Section 6 concludes with some concrete suggestions for future MAP reduction researchers based on the results of our analyses.
...
Our meta-analysis of `r num_studies` policy-relevant interventions yields an overall meta-analytic effect of `rmain_result$Delta` (se = `r main_result$se`), p =`r main_result$pval`. This is not a clinically or statistically significant effect.
...
publication_type_eff_size <- dat |> split(~pub_status) |> 
  map(count_and_robust); publication_type_eff_size
```
...
As this graph suggests, we see systematically smaller results in reports by advocacy organizations than in other types of publication. This is consistent with the contention that academic papers are subject to pressure to publish significant results, but not conclusively. 

```{r publication_type_table, include=F, echo=F}
publication_type_eff_size <- dat |> split(~pub_status) |> 
 map(count_and_robust); publication_type_eff_size
```
...
| Approach              | N (studies)                                                    | DELTA (se)                                                                                                                                                                                                                                                                                                                                                           |
|------------|------------|-------------------------------------------------|
| Advocacy organization | `r publication_type_eff_size$'Advocacy organization'$N_unique` | `r publication_type_eff_size$'Advocacy organization'$Delta` (`r publication_type_eff_size$'Advocacy organization'$se`)`r ifelse(publication_type_eff_size$'Advocacy organization'$pval < 0.05, "*", ifelse(publication_type_eff_size$'Advocacy organization'$pval < 0.01, "**", ifelse(publication_type_eff_size$'Advocacy organization'$pval < 0.001, "***", "")))` |
| Journal article       | `r publication_type_eff_size$'Journal article'$N_unique`       | `r publication_type_eff_size$'Journal article'$Delta` (`r publication_type_eff_size$'Journal article'$se`)`r ifelse(publication_type_eff_size$'Journal article'$pval < 0.05, "*", ifelse(publication_type_eff_size$'Journal article'$pval < 0.01, "**", ifelse(publication_type_eff_size$'Journal article'$pval < 0.001, "***", "")))`                               |
| Preprint              | `r publication_type_eff_size$Preprint$N_unique`                | `r publication_type_eff_size$Preprint$Delta` (`r publication_type_eff_size$Preprint$se`)`r ifelse(publication_type_eff_size$Preprint$pval < 0.05, "*", ifelse(publication_type_eff_size$Preprint$pval < 0.01, "**", ifelse(publication_type_eff_size$Preprint$pval < 0.001, "***", "")))`                                                                            |
| Thesis                | `r publication_type_eff_size$Thesis$N_unique`                  | `r publication_type_eff_size$Thesis$Delta` (`r publication_type_eff_size$Thesis$se`)`r ifelse(publication_type_eff_size$Thesis$pval < 0.05, "*", ifelse(publication_type_eff_size$Thesis$pval < 0.01, "**", ifelse(publication_type_eff_size$Thesis$pval < 0.001, "***", "")))`                                                                                      |

We see that all effect sizes are clustered around small, insignificant results, except for the advocacy organization category, which show a very small backlash on average (p = `r publication_type_eff_size$'Advocacy organization'$pval`).
...
within_journal_article_bias[2,1]
```

These four categories of publication -- preprint, jounral article, theses (both bachelor's and graduate), and advocacy organization materials -- are subject to different processes and incentives. One way to account for this in a check for publication bias is to look for a relationship between standard errors and effect sizes within journal articles, which would be evidence for the "file drawer problem" [@rosenthal1979]. We find no such relationship (b = `r within_journal_article_bias[2,1]` (se = `r within_journal_article_bias[2,2]`), p = `r within_journal_article_bias[2,4]`).

Overall, this literature is not shy of publishing null results, to its credit. However, this may partly be a product of our focus on MAP consumption as a dependent variable. The typical paper in this literature presents multiple dependent variables and/or subgroup analyses, and often leads with the largest results. @allen2002, for example, report that "red and white meat consumption for both treatment and control groups remained constant" (p. 124), but write in their abstract that "most participants" in the treatment group reported less favorable impressions of meat, less "object identification" with meat, and greater self-reported consumption of fruits and vegetables three weeks later (p. 118). Likewise, @piester2020, over three studies, find two null effects and one significantly positive effect on MAP consumption overall, but find that women typically responded to treatment while men did not, and present gender-based results in their abstract. It is possible that we would find evidence of publication bias if we looked at a wider range of dependent variables and studies.
...
| Approach       | N (studies)                         | Glass's DELTA (se)                                                                                                                                                                                                            |
|------------|------------|-------------------------------------------------|
...
| Method of Delivery                 | N (studies)                      | Glass's DELTA (se)                                                                                                                                                                                             |
|-------------|-------------|-----------------------------------------------|
...
```{r animal_advocacy_org_materials, echo=F, include=F}
advocacy_results <- dat |> mutate(advocacy_materials = if_else(advocacy_org == "N", F, T)) |> 
  split(~advocacy_materials) |> map(count_and_robust); advocacy_results 

advocacy_org_specific_results <- dat |> split(~advocacy_org) |> map(count_and_robust); advocacy_org_specific_results
```

As suggested earlier by our tests for publication bias, evaluations of materials provided by animal advocacy organizations are not effective at reducing MAP consumption. `r advocacy_results$'TRUE'['N_unique']` studies were either conducted by animal advocacy organizations or used their materials; these have an pooled effect size of `r advocacy_results$'TRUE'$Delta` (se = `r advocacy_results$'TRUE'$se`), p = `r advocacy_results$'TRUE'$pval`. In other words, a miniscule backlash.By contrast, `r advocacy_results$'FALSE'['N_unique']` studies were not conducted by animal advocacy organizations or use their materials, and have a pooled effect size of `r advocacy_results$'FALSE'$Delta` (se = `r advocacy_results$'FALSE'$se`), p = `r advocacy_results$'FALSE'$pval`. While we consider both of these results to be null, we observe that with the animal advocacy organizations subtracted, the remaining studies are statistically significant at p < 0.01.

Table XXX displays the effect sizes associated with the materials of each animal advocacy organization in our dataset separated by the providing organization. Where N = 1, the estimate is not a meta-analytic estimate, but the results from that single study.

| Organization        | N (studies) | ∆ (se) |
|---------------------|-------------|------------|
| NA (Academic)       | `r advocacy_org_specific_results$N[['N_unique']]`                    | `r advocacy_org_specific_results$N[['Delta']]` (`r advocacy_org_specific_results$N[['se']]`)`r get_significance_stars(advocacy_org_specific_results$N[['pval']])` |
| Humane League       | `r advocacy_org_specific_results$'Humane League'[['N_unique']]`       | `r advocacy_org_specific_results$'Humane League'[['Delta']]` (`r advocacy_org_specific_results$'Humane League'[['se']]`)`r get_significance_stars(advocacy_org_specific_results$'Humane League'[['pval']])` |
| Vegan Outreach      | `r advocacy_org_specific_results$'Vegan Outreach'[['N_unique']]`     | `r advocacy_org_specific_results$'Vegan Outreach'[['Delta']]` (`r advocacy_org_specific_results$'Vegan Outreach'[['se']]`)`r get_significance_stars(advocacy_org_specific_results$'Vegan Outreach'[['pval']])` |
| Animal Rights Watch | `r advocacy_org_specific_results$'Animal Rights Watch'[['N_unique']]` | `r advocacy_org_specific_results$'Animal Rights Watch'[['Delta']]` (`r advocacy_org_specific_results$'Animal Rights Watch'[['se']]`)`r get_significance_stars(advocacy_org_specific_results$'Animal Rights Watch'[['pval']])` |
| Animals Australia   | `r advocacy_org_specific_results$'Animals Australia'[['N_unique']]`   | `r advocacy_org_specific_results$'Animals Australia'[['Delta']]` (`r advocacy_org_specific_results$'Animals Australia'[['se']]`)`r get_significance_stars(advocacy_org_specific_results$'Animals Australia'[['pval']])` |
| Faunalytics         | `r advocacy_org_specific_results$Faunalytics[['N_unique']]`            | `r advocacy_org_specific_results$Faunalytics[['Delta']]` (`r advocacy_org_specific_results$Faunalytics[['se']]`)`r get_significance_stars(advocacy_org_specific_results$Faunalytics[['pval']])` |
| Mercy For Animals   | `r advocacy_org_specific_results$'Mercy For Animals'[['N_unique']]`   | `r advocacy_org_specific_results$'Mercy For Animals'[['Delta']]` (`r advocacy_org_specific_results$'Mercy For Animals'[['se']]`)`r get_significance_stars(advocacy_org_specific_results$'Mercy For Animals'[['pval']])` |
Table: * < 0.05, ** < 0.01, *** < 0.001.

[table XXX about here]

On the one hand, we applaud these organizations for making their materials freely available and for publishing null results when they find them. It might also be the case that these groups' most effective campaigns or materials have not yet been evaluated, or do not lend themselves well to evaluation (e.g. corporate campaigns). However this overall backlash result suggests that the approaches, delivery mechanisms, and specifics of the materials these organizations provide to others for re-use are in need of a rethink.

### self-report measures are not systematically different

```{r self_report, echo=F, include=F}
self_report_results <- dat |> split(~self_report == 'Y') |> map(count_and_robust); self_report_results
```
Self-reported outcomes are a well-understood potential source of bias [@hebert1995social; @hebert1997gender; @cerri2019; @schoeller2013; @mathur2021effectiveness], and we expected to see evidence of this bias manifesting as larger results for self-reported outcomes than for obliquely measured results. However, we find the opposite: The `r self_report_results$'TRUE'$N_unique ` studies with self-report outcomes have an average effect size of `r self_report_results$'TRUE'$Delta` (se = `r self_report_results$'TRUE'$se`), p = `r self_report_results$'TRUE'$pval`, while the `r self_report_results$'FALSE'$N_unique` studies with obliquely measured outcomes have an average effect size of `r self_report_results$'FALSE'$Delta` (se = `r self_report_results$'FALSE'$se`), p = `r self_report_results$'FALSE'$pval`.

```{r self_report_without_advocacy_studies, echo=F, include=F}
dat |> filter(advocacy_org == 'N') |> split(~self_report == 'Y') |> map(count_and_robust)
```

We do not read too much into this, as there are numerous systematic differences between the two groups of studies. For instance, all of the interventions that use materials produced by advocacy organizations, which we've seen produce a backlash effect, use self-report measures. (If we remove those studies from our analysis, the studies with obliquely measured outcomes still produce a larger pooled effect). Unfortunately, no studies in our sample provide both self-reported and obliquely measured MAP consumption outcomes, so we cannot assess how well one outcome predicts the other within a given study.
...
We record two delay variables: one for time elapsed between the start of intervention and the last measurement, and one for time elapsed between the end of intervention and last measurement. In both cases, we find a strict null relationship between days of delay and effect size.

### country?

```{r eff_size_by_country, echo=F, include=F}

country_results_data <- dat |> split(~country) |> 
  map(count_and_robust); country_results_data
```

Table XXX presents results separated by the country in which they took place.

| Country                                             | N_unique | Delta (se)                                         |
|-----------------------------------------------------|----------|----------------------------------------------------|
| Australia                                           | `r country_results_data$Australia$N_unique` | `r country_results_data$Australia$Delta` (`r country_results_data$Australia$se`) |
| Canada                                              | `r country_results_data$Canada$N_unique`    | `r country_results_data$Canada$Delta` (`r country_results_data$Canada$se`) |
| Denmark                                             | `r country_results_data$Denmark$N_unique`   | `r country_results_data$Denmark$Delta` (`r country_results_data$Denmark$se`) |
| Germany                                             | `r country_results_data$Germany$N_unique`   | `r country_results_data$Germany$Delta` (`r country_results_data$Germany$se`) |
| Netherlands                                         | `r country_results_data$Netherlands$N_unique` | `r country_results_data$Netherlands$Delta` (`r country_results_data$Netherlands$se`) |
| Sweden                                              | `r country_results_data$Sweden$N_unique`    | `r country_results_data$Sweden$Delta` (`r country_results_data$Sweden$se`) |
| United Kingdom                                      | `r country_results_data$'United Kingdom'$N_unique` | `r country_results_data$'United Kingdom'$Delta` (`r country_results_data$'United Kingdom'$se`) |
| United States                                       | `r country_results_data$'United States'$N_unique` | `r country_results_data$'United States'$Delta` (`r country_results_data$'United States'$se`) |
| United States, United Kingdom, Canada, Australia, and other | `r country_results_data$'United States, United Kingdom, Canada, Australia, and other'$N_unique` | `r country_results_data$'United States, United Kingdom, Canada, Australia, and other'$Delta` (`r country_results_data$'United States, United Kingdom, Canada, Australia, and other'$se`) |
Table: * < 0.05, ** < 0.01, *** < 0.001.

### meat vs MAP

```{r meat_vs_MAP, echo=F, include=F}
meat_vs_map <- dat |> split(~str_detect(
  pattern = "MAP", dat$outcome_category)) |>
  map(count_and_robust); meat_vs_map
  
```

The majority of studies in our database (`r meat_vs_map$'FALSE'$N_unique` of `r num_studies`) looked at reducing consumption of meat specifically, while the remaining `r meat_vs_map$'TRUE'$N_unique` studies looked at reducing consumption of meat and other animal products (typically eggs). The average effect size for the former is `r meat_vs_map$'FALSE'$Delta` (se = `r meat_vs_map$'FALSE'$se`), p = `r meat_vs_map$'FALSE'$pval`, while the average effect size for the latter is `r meat_vs_map$'TRUE'$Delta` (se = `r meat_vs_map$'TRUE'$se`), p = `r meat_vs_map$'TRUE'$pval`.

### Robustness to markers of study quality
```{r robustness_to_study_quality, echo=F, include=F}
pap_split_results <- dat |> 
  mutate(has_pap = if_else(public_pre_analysis_plan == 'N', F, T)) |>
  split(~has_pap) |> map(count_and_robust); pap_split_results

open_data_results <- dat |> 
  mutate(has_open_data = if_else(open_data == 'N', F, T)) |>
  split(~has_open_data) |> map(count_and_robust); open_data_results
```
...
```{r open_science_practice_results, echo=F, include=F}

pap_study_results <- dat |> 
  filter(public_pre_analysis_plan != 'N') |> 
  count_and_robust(); pap_study_results
  
open_data_results <- dat |> 
  filter(open_data != 'N') |> 
  count_and_robust(); open_data_results
  
pap_and_open_data_results <- dat |>
  filter(public_pre_analysis_plan != 'N',
         open_data != 'N') |> 
  count_and_robust(); pap_and_open_data_results

neither_pap_nor_open_data_results <- dat |>
  filter(public_pre_analysis_plan == 'N',
         open_data == 'N') |> 
  count_and_robust(); neither_pap_nor_open_data_results
```

Table XXX displays the effect sizes of studies with a public pre-analysis plan, open data, both, and neither.

| Open Science Practice            | N_unique | Delta (se)                                                     |
|----------------------------------|----------|----------------------------------------------------------------|
| Pre-analysis Plan                | `r pap_study_results$N_unique` | `r pap_study_results$Delta` (`r pap_study_results$se`)`r get_significance_stars(pap_study_results$pval)` |
| Open Data                        | `r open_data_results$N_unique` | `r open_data_results$Delta` (`r open_data_results$se`)`r get_significance_stars(open_data_results$pval)` |
| Both                             | `r pap_and_open_data_results$N_unique` | `r pap_and_open_data_results$Delta` (`r pap_and_open_data_results$se`)`r get_significance_stars(pap_and_open_data_results$pval)` |
| Neither                          | `r neither_pap_nor_open_data_results$N_unique` | `r neither_pap_nor_open_data_results$Delta` (`r neither_pap_nor_open_data_results$se`)`r get_significance_stars(neither_pap_nor_open_data_results$pval)` |
Table: * < 0.05, ** < 0.01, *** < 0.001.
...
# prop_stronger_result <- 
#   MetaUtility::prop_stronger(q = 0.2,
...
-   price interventions! constraints vs. preferences, focus on constraints! (Tabarrok piece on crime)
  - also this piece found nudges didn't work but prices were much more important https://www.ncbi.nlm.nih.gov/pmc/articles/PMC9021786/\
...
- see "Unsustainable, unhealthy, or disgusting? Comparing different persuasive messages against meat consumption
Meat eaters by dissociation: How we present, prepare and talk about meat increases willingness to eat meat by reducing empathy and disgust" and "Unsustainable, unhealthy, or disgusting? Comparing different persuasive messages against meat consumption" for discussion
...
imagine that we magically were able to scale these interventions up to the entire western world .2 Policy lessons? - don't run a leaflet study and expect it to work. We need new leaflets or new approaches. - Ditto with online - If you're a college administrator at an elite west coast college, the Jalil et al stuff is a great start,but awaiting replication. 10 classrooms is not very much. - no handle whatsoever on what gets people to change outside of a dining hall
