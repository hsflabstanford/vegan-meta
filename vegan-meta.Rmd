...
By these standards, `r nrow(nudge_studies)` intervention in our database are nudges printed in `r length(unique(nudge_studies$doi_or_url))` papers. A full `r hybrid_nudge_approach_number` of these studies are hybrid approaches. `r nrow(norms_nudge_approaches)` of these, as previously discussed, are norms-based nudges. The remaining `r nrow(nudge_studies) - nrow(norms_nudge_approaches)` are nudges that do not have a norms-based component. 
