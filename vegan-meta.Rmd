...
publication_type_eff_size <- dat |> split(~pub_status) |> 
  map(count_and_robust) |> bind_rows(.id = "Publication type") |> mutate(
    `∆ (se)` = sprintf("%.3f%s (%.3f)", Delta, get_significance_stars(pval), se)
  ) |>
  select(`Publication type`, N_unique, `∆ (se)`)  # Select only necessary columns

publication_type_eff_size_gt <- publication_type_eff_size |>
  gt() |>
  tab_header(
    title = "Table XXX",
    subtitle = "Differences in Average Effect Size by Publication Type"
  ) |>
  cols_label(
...
`r publication_type_eff_size_gt`
