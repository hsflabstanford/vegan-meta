...
publication_type_eff_size <- dat |> split(~pub_status) |> 
  map(count_and_robust) |> 
  bind_rows(.id = "publication_type"); publication_type_eff_size

publication_type_eff_size_reformatted <- publication_type_eff_size |> 
  mutate(delta_se = sprintf("%.3f%s (%.3f)", Delta, get_significance_stars(pval), se)) |>
  select(publication_type, N_unique, delta_se) # Select only necessary columns

publication_type_eff_size_gt <- publication_type_eff_size_reformatted |>
  gt() |>
  tab_header(
    title = "Table XXX",
...
all_results_gt_table <- all_results_formatted |>
  gt() |>
  tab_header(
    title = "Summary of Results by Theory/Approach",
...
advocacy_results <- dat |>
  mutate(advocacy_materials = if_else(advocacy_org == "N", "Researcher materials", "Advocacy organization materials")) |>
  split(~advocacy_materials) |>
  map(count_and_robust) |>
  bind_rows(.id = "Intervention_type"); advocacy_results

advocacy_org_specific_results <- dat |>
  split(~advocacy_org) |>
  map(count_and_robust)|>
  ...
  mutate(
    advocacy_org = if_else(advocacy_org == "N", 
                           ...
                           advocacy_org),
    delta_se = sprintf("%.3f%s (%.3f)", Delta, get_significance_stars(pval), 
                       se)) |>
  select(advocacy_org, N_unique, delta_se); advocacy_org_specific_results

advocacy_org_specific_table <- advocacy_org_specific_results |> 
  gt() |> 
  tab_header(
    title = "Table XXX",
    subtitle = "Effect sizes associated with animnal advocacy organizations"
  )
```

As suggested earlier by our tests for publication bias, evaluations of materials provided by animal advocacy organizations are not effective at reducing MAP consumption. `r sum(advocacy_results$N_unique[advocacy_results$Intervention_type == "Advocacy organization materials"])` studies were either conducted by animal advocacy organizations or used their materials; these have a pooled effect size of `r advocacy_results$Delta[advocacy_results$Intervention_type == "Advocacy organization materials"]` (se = `r advocacy_results$se[advocacy_results$Intervention_type == "Advocacy organization materials"]`), p = `r advocacy_results$pval[advocacy_results$Intervention_type == "Advocacy organization materials"]`. In other words, a minuscule backlash. By contrast, `r sum(advocacy_results$N_unique[advocacy_results$Intervention_type == "Researcher materials"])` studies were not conducted by animal advocacy organizations or used their materials, and have a pooled effect size of `r advocacy_results$Delta[advocacy_results$Intervention_type == "Researcher materials"]` (se = `r advocacy_results$se[advocacy_results$Intervention_type == "Researcher materials"]`), p = `r advocacy_results$pval[advocacy_results$Intervention_type == "Researcher materials"]`. While we consider both of these results to be null, we observe that with the animal advocacy organizations subtracted, the remaining studies are statistically significant at p < 0.01.
