...
### 3.4 a diversity of dependent variables

```{r dependent_variable_tab, echo=F, include=F}
self_report_data <- dat |> sum_tab(self_report)
```
Most studies provide self-reported data: `r self_report_data['Y']` of `r num_studies`. 
## 5 Meta-analysis
...
In our sample, `r sign_table[[2]]` of `r num_studies` had, in the words of the relevant paper, no stiatistically significant effects (`r null_proportion` percent of all studies).`r sign_table[[1]]` studies had a statistically significant positive effect, while`r sign_table[[3]]` had a significant backlash effect.
...
The variety of dependent variables
...
### place-based vs general interventions
...
We divide this into two sections: for researchers, and for policymakers.
For researchers?
1. We find very little overall.
    -   this is in contrast to previous reviews. however it's well in line with some contemporary pieces that find
      - RCT don't scale (@dellavigna2022)
      -   behavioral change is typically weaker than attitude change or updating [@haaland2023]
      -   self-reported intentions don't predict behavior [@kormos2014; hassan2016]
      -   attitudes don't predict behavior [@porat2024]
      -   the kinds of structural interventions that actually change deep-seated habits are hard to evaluate in an RCT, e.g. @stevenson2023: "When it comes to the type of limited-scope interventions that lend themselves to high-quality evaluation, social change is hard to engineer."

2. how to interpret the distance between general effects and RPMC effects?
...
3.     -   So where does research go from here?
...
For policymakers:        
1. How concerned should a policymaker be about this discrepancy? That depends on your goals.
  - for environment and health, RPMC is probably the high-value target, and any reduction is good.
  - For animal welfare,this is bad [@mathur2022ethical]
