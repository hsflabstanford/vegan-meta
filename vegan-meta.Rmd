...
-   TODO: figure out how to structure this in a way that is interesting to the reader. possibly, the entire thing will go in the appendix.
...
dat |> filter(str_detect(dat$outcome, "red meat") | str_detect(dat$outcome, "processed meat")) 
 

```

Our meta-analysis of `r num_studies` policy-relevant interventions yields an overall meta-analytic effect of `r`main_result$Delta` (se = `rmain_result$se`), p =`r main_result\$pval\`. This null finding stands in marked contrast to previous reviews, which found [PUT IN RESULTS OF PREVIOUS METAS]. This diverge seems to be a product of our inclusion criteria, specifically the requirement that studies measure actual consumption behavior rather than self-reported intentions or hypothetical choices. [say more about this].

In our sample, `r sign_table[[2]]` of `r num_studies` had, in the words of the relevant paper, no stiatistically significant effects (`r`null_proportion`percent of all studies).v`r sign_table[[1]]`studies had a statistically si positive effect,`r sign_table[[3]] \` had a significant backlash effect.
...
-   We find very little overall.
    -   this is in contrast to previous reviews
    -   however it's well in line with some contemporary pieces that find
        -   behavioral change is typically weaker than attitude change or updating
        -   self-reported intentions don't predict behavior
        -   attitudes don't predict behavior
        -   the kinds of structural interventions that actually change deep-seated habits are hard to evaluate in an RCT
-   how to interpret the distance between general effects and RPMC effects?
    -   The core unknown here is how much of this gap is being driven by substitution to other MAP products and how much
        -   Our working theory is that reductions in RPMC are driven somewhat, but not entirely, by substitution to other MAP products.
            -   [Emmons et al. (2005a)](https://pubmed.ncbi.nlm.nih.gov/15941955/) is the best evidence we have of it, where the researchers find a reduction in red meat but no attendant increase in fruit and vegetable intake. However, [Emmons et al. (2005)](https://pubmed.ncbi.nlm.nih.gov/15933240/) find reductions in red meat accompanied by increases in fruit and vegetable consumption. [Sorensen et al. (2005)](https://pubmed.ncbi.nlm.nih.gov/16006422/) finds increases in fruit and vegetable intake as well (along with RMC reductions). The three Carfora et al. papers measure the effects of text messages on RPMC only measure RPMC.
            -   Jalil et al., as always the standout study, report "the probability of **beef**, **poultry + fish**, and **meat** fell by 1.9, 3.9 and 5.6 percentage points (p.p.), respectively, whereas the probability of veg increased by 4.1 p.p." Which I guess means that "meat" is just the sum of the other two categories.
            ...
            -   So we are left with intuition. Ours is that we should seriously downweight any RPMC estimates if we're trying to estimate overall reduction on MAP consumption — if I had to pick a number, I'd say 60% of the effect sizes are driven by inter-MAP substitution.
            -   How concerned should a policymaker be about this? That depends on your goals.
                -   for environment and health, RPMC is probably the high-value target, and any reduction is good.
                -   For animal welfare,this is a disaster [@mathur2022ethical]
                -   Moreover, imagine that we magically were able to scale these interventions up to the entire western world
...
in terms of power analyses, substantially revise your expectations downwards. e.g. Binachi 2022 is powered to detect d = 0.6. Try an order of magnitude smaller PRICE STUDIES Look at Garnett studies and also [Vellinga et al. (2022)](https://doi.org/10.1186/s12889-022-13535-9)
