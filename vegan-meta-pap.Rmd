...
dat <- data.frame(unique_study_id = 1:24,
                  ...
                  eff_type = sample(c("d_i_m", "d_i_p", "d_i_d"), 24, T),
                  n_t_post = sample(1:100, 24, replace = T),
                  n_c_post = sample(1:100, 24, replace = T),
                  theory = as.factor(sample(
                    c("animal welfare", "environment", "psychology"), 
                    24, replace = T)),
                  cluster_var = as.logical(rbinom(24, 1, 0.4)),
...
### 4.6 any relationship between publication date and effect size or validity?

```{r publication_date}

dat |> sum_lm(d, year)

dat |>  mutate(decade = as.factor(
      case_when(
        year <= 1989 ~ "1980s",
