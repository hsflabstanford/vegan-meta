...
make a dataset
```{r make_dataset}
dat <- data.frame(unique_study_id = 1:16,
                  ...
                  eff_type = sample(c("d_i_m", "d_i_p", "d_i_d"), 16, T),
                  n_t_post = sample(1:100, 16, replace = T),
                  n_c_post = sample(1:100, 16, replace = T),
                  theory = as.factor(sample(
                    c("animal welfare", "environment", "psychology"), 
                    16, replace = T)),
                  cluster_var = as.logical(rbinom(16, 1, 0.4)),
                  self_report = as.logical(rbinom(16, 1, 0.2)),
                  leaflet     = as.logical(rbinom(16, 1, 0.2)),
                  doi_or_url = as.logical(rbinom(16, 1, 0.2)),
...
### 4.3 Social desirability penalty
...
dat |>  mutate(d = 
                 if_else(self_report,
                         d - 0.1,
                         d)) |> map_robust()
#' I would prefer that self_report be a T/F where T has a 90% chance of occuring,
#' but I'm not seeing a <5 minutes solution to doing this 
```
...
### 4.5 Any relationship between delay and outcome?
```{r delay}
dat |> sum_lm(d, delay)
...
### 4.6 record computational environment
```{r reproducibility}
