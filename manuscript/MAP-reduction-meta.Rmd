...
Table 1 summarizes studies in terms of population, location,  decade of publication, where studies were published, and how interventions were delivered. The earliest paper was published in 2002 [@allen2002] and a majority (`r  decade_tab |> filter(decade == "2020s") |> pull(n)` of `r num_papers` papers) have been published since 2020.
...
summary_stats_table |> 
  footnote("Delivery Method Ns will not tally to 40 because some studies employ multiple delivery methods.")
```

## Constituent Theories
[OR A LINKING SENTENCE?]
...
In our dataset, the pooled effect of all interventions is $\Delta$ = `r overall_results$Delta`, with 95% confidence intervals of `r overall_results$CI`, and a p-value of `r overall_results$p_val`.

```{r meta_table}
source('./scripts/meta-table.R')
meta_table |>  footnote("Types of persuasion Ns will not total to the Ns for persuasion overall because many studies employ different # kinds of arguments together.")
```
