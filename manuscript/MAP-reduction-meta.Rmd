...
First, we separated studies looking at reductions in red and processed meat from everything else.
Our initial draft took these at face value as estimates of net MAP reduction.
...
Second, and related, we initially included searched for studies that were not necessarily aimed at achieving overall MAP reduction, but rather a generally healthier diet with some amount of chicken and/or fish, for instance the Mediterranean diet.
Many of these studies had measurement strategies that would otherwise have qualified them, i.e they measured MAP consumption overall or measured consumption in many categories.
...
Arguably the hardest decision in meta-analysis is what studies to include or exclude. By far the most common reason was the wrong dependent variable (e.g. attitudinal or intentional outcomes). However, many cases were harder and required some discussion. Here were a few difficult cases.

Some studies limit dietary portions or switch what people are served (e.g. children being served more vegetables at lunchtime). We did not include these studies because they were essentially guaranteed to have effects that were either positive or at least bounded at zero. However, we did include studies that provided free access to meat alternatives [@acharya2004; @bianchi2022] and measured outcomes after the intervention had concluded.

Other studies induce a form of treatment in their control groups, for instance asked to take a pledge to go vegetarian or encouraged not to change one's diet over the course of study, which could potentially induce changes relative to baseline consumption patterns. We only analyzed studies where with a pure control group or an unrelated placebo.

Another common design limitation we encountered was treatment assigned at the level of alternating weeks at a cafeteria. Generally these studies did not have enough weeks to meet our sample size requirements, but we also noted that simply alternating weeks is not random assignment and it is possible that consumption patterns will not be equivalent between treatment and control groups in expectation.

Last, we encountered many studies that measured fruit and or/vegetable consumption but not MAP consumption. In some cases, it might have been possible to make some strong assumptions about substitution and estimate a MAP effect size, but we decided to focus on studies that reported this information directly.
...
The canonical definition of nudge [@thaler2009] is "any aspect of the choice architecture that alters people’s behaviour in a predictable way, without forbidding any options or significantly changing their economic incentives. 
To count as a mere nudge, the intervention must be easy and cheap to avoid" (p. 6), while others [@hausman2010] define nudges as "ways of influencing choice without limiting the choice set or making alternatives appreciably more costly in terms of time, trouble, social sanctions, and so forth" (p. 126).
...
A future project might investigate exactly what reactions are occurring by asking subjects how well they recall a quick norms message and what it made them think about.
A high prevalence of subjects who are unable to recall the message's specifics but nevertheless reveal treatment effects would be evidence that norms are acting through automatic rather than reflective processes.

### A lack of follow-up data in cafeteria-based interventions

One potential source of measurement error that we cannot easily correct for is the possibility that a subject who is guided towards eating less meat at one meal eats more at the next, i.e. "regression to the meat." 
Most cafeteria-based studies we looked at were not designed to measure this kind of spillover effect.
We encourage researchers in this field to collect follow-up outcomes whenever possible.
