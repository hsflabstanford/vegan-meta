...
merged_model <- merged_dat |> robu(formula = d ~ 1, studynum = unique_study_id, 
                 var.eff.size = var_d, modelweights = 'CORR', small = TRUE)
merged_results <- extract_model_results(data = merged_dat)
merged_tau <-  round(sqrt(merged_model$mod_info$tau.sq), 3)

```

By contrast, studies that only attempted to reduce consumption of RPM had larger estimates: across these 17 studies and 25 estimates, we detect a pooled effect of $\Delta$ = `r round(rpmc_model$reg_table$b.r, 3)` (95% CI: `r rpmc_results$CI`), p = `r rpmc_results$p_val`, $\tau$ = `r red_tau`.
We estimate that `r round(red_high_prop_test$est * 100, 2)`% of true effects are above $\Delta$ = 0.2.

Meta-analyzing the MAP and RPM studies combined yields an overall $\Delta$ of `r round(merged_model$reg_table$b.r, 3)` (95% CI: `r merged_results$CI`), p = `r merged_results$p_val`, $\tau$ = `r merged_tau`.
