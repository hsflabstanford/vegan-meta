...
```{r dat_setup, eval=FALSE, echo=FALSE}
library(dplyr)
...
dat <- read.csv('./data/vegan-meta.csv') |> group_by(title) |> 
  ...
  ungroup() |>
  mutate(decade = cut(year, breaks = c(2000, 2010, 2020, 2025), 
                      labels = c("2000s", "2010s", "2020s"), right = FALSE),
