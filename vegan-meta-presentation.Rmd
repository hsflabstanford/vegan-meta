...
-   Meta-analytic effect size: $\Delta$ = `r model$Delta` (SE = `r model$se`), p = `r model$pval`.
...
-   RPM reduction interventions outperform general MAP interventions: $\Delta$ = `r rpmc_model$Delta` (SE = `r rpmc_model$se`), p = `r rpmc_model$pval`.
-   Focus primarily on health benefits.
-   Lacking data on substitution to white meat and fish.
...
-   Unclear limiting principles for norms' effectiveness 
-   see @sparkman2020 for standout example and @Weikertova2024 for a review & meta-analysis

## The Evidence for Nudges on MAP Consumption Is Scant

-   Nudge and diet studies is a huge literature...
-   but only two nudge studies [@andersson2021; @kanchanachitra2020] met our inclusion criteria.
...
-   Pooled effect size for health-based persuasion: $\Delta$ = `r health_model$Delta` (SE = `r health_model$se`), p = `r health_model$pval`.
-   More effective in RPM interventions than in general MAP reduction.

## Environmental Appeals Have Modest Positive Effects

-   Pooled effect size for environmental appeals: $\Delta$ = `r environment_model$Delta` (SE = `r environment_model$se`), p = `r environment_model$pval`.
-   Strongest evidence from a study on student dining behavior [@jalil2023] which repalced an intro econ lecture for students at a small liberal arts college with a lecture on the environmental and health reasons to cut back on meat -- and then tracked student dining choices for three years
    - Overall reduction of 5.4%
...
- my email is setgree@stanford.edu
