...
(Also the intervention can't just be serving people less meat)
...
`r third_decade_papers` of `r num_papers` papers were published 2020-2023

We disqualified about 400 studies and counting, and Jacob Peacock is working on a parallel review on the literature as a whole
...
\begin{columns}
    \begin{column}{0.6\textwidth}
            \texttt{`r intervention_types_counts$leaflet_count`} employed leaflets
            \texttt{`r intervention_types_counts$video_count`} used videos
            \texttt{`r intervention_types_counts$cafeteria_or_restaurant`} interventions in a cafeteria or restaurant
            \texttt{`r intervention_types_counts$internet_based`} were administered online
            \texttt{`r pop_dat["university students"]`} with university students
    \end{column}
...
## Landmark study: Sparkman et al. (2020)
