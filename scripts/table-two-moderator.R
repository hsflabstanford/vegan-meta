...
moderator_table <- bind_rows(
  ...
  kbl(
    ...
    col.names = c("Study Characteristic", "N (Studies)", "N (Estimates)", 
                  "SMD", "95\\% CIs", "\\shortstack{Subset \\\\ $p$ value}", 
  ...
  add_footnote("Moderation analyses by differences in outcomes, population, region, decade of publication, and delivery method.", 
               notation = "none")

# took out The first p value column tests the hypothesis that the subset of studies with a given characteristic is significantly different than an SMD of zero. The second compares effects within a given group, with the top category set to reference.", 