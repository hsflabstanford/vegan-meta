...
[^1]: This study was formally published in 2024, but was available as a preprint in our search window.
    Our excluded studies also include some papers that were published outside of our window, but if they met our inclusion criteria, we looked for time-eligible preprints.
...
pred_lower <- round(model$reg_table$b.r - 
                      qt(0.975, df) * sqrt(model$reg_table$SE^2 +
...
Given the pooled effect size and the estimated heterogeneity, we estimate that `r round(low_prop_test$est * 100)`% of true effects are above SMD = 0.1, and just `r round(high_prop_test$est * 100)`% are above SMD = 0.2 [@mathur2019; @mathur2020robust]. We estimate prediction intervals ranging from  `r pred_lower` to `r pred_upper`, indicating uncertainty about the anticipated direction of a hypothetical future study but relative certainty that the effect size is likely to be small to moderate in magnitude. 
