...
Our code and data are shared on GitHub (\url{https://github.com/hsflabstanford/vegan-meta}), and Code Ocean (\url{https://doi.org/10.24433/CO.6020578.v5}). The Code Ocean repository allows for reproducing our papers' results without downloading any files or managing software versions.
...
In our main dataset, we recorded `r `overall_results$N_estimates - no_unspecified_nulls$N_estimates` point estimates as 'unspecified nulls' because we lacked the statistical information to calculate a precise effect size. We then manually coded these estimates as 0.01, a convention borrowed from [@porat2020]. If we set these to be equal to 0.00, our meta-analytic estimate remains `r zero_delta`. If we omit them entirely, our overall estimate rises to `r no_unspecified_nulls$Delta` (95)
