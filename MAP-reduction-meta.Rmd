...
However, many of these prior reviews investigated particular theoretical perspectives, for example choice architecture [@bianchi2018restructuring], appeals to animal welfare @mathur2021effectiveness], or literacy interventions [@DiGennaro2024], rather than comparing the efficacy of different approaches.
...
This small effect may come as a surprise to readers of previous reviews, which found more positive results [@mathur2021meta; @chang2023].
We attribute this difference to our inclusion criteria.
...
According to their papers, `r effect_size_table['0']` of `r num_interventions` interventions had null effects.
However, many studies present a variety of wide variety of outcomes and lead with, e.g., a gender-moderated effect [@piester2020] or the relationship between an underlying attitude measure and consumption outcomes [@Berndsen2005], or where meat reduction was just one of many strategies suggested as part of a broader program [@abramahse2007].

-   This section depends on what model we choose - note the preponderance of nulls according to the studies themselves - some weak evidence of publication bias (effect size goes up as SE gets bigger)
...
dat |> split(~pub_status_one) |> map(map_robu)
```
