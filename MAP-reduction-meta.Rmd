...
```{r table_one}
# eight cagtegories: overall, nudge, norm, norms + persuasion,
# persuasion (overall), persuasion (health), persuasion (environment), and 
# persuasion (animal welfare). each gets its own model.
# the model has all the info we need

# TODO: custom function that does ∆*** (SE) or whatever.
...
# persuasion overall
persuasion_overall <- dat |> filter(theory == 'persuasion') |> map_robu()

# persuasion (health)
health_persuasion <- dat |> filter(str_detect(secondary_theory, 'health')) |> map_robu()

# persuasion (environment)
enviroment_persuasion <- dat |> filter(str_detect(secondary_theory, 'environment')) |> map_robu()

# persuasion (health)
animal_persuasion <- dat |> filter(str_detect(secondary_theory, 'animal')) |> map_robu()
...
table_one <- tibble(
  ...
  "N (Studies)" = c(model$N_studies, nudge_model$N_studies, NA, NA, NA, NA, NA, NA),
