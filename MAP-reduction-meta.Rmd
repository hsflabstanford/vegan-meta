...
calculate_n_total <- function(data) {
  summarise(data, n_total = plyr::round_any(sum(n_t_total_pop + n_c_total_pop), 100, floor))
}
...
# Table One
table_one <- tibble(
  Approach = c("\\textbf{Overall}", "Norms", "Nudge", "Persuasion", "Norms + Persuasion"),
  "N (Studies)" = c(model$N_studies, norms_model$N_studies, nudge_model$N_studies, persuasion_model$N_studies, norms_persuasion_model$N_studies),
  "N (Interventions)" = c(model$N_interventions, norms_model$N_interventions, nudge_model$N_interventions, persuasion_model$N_interventions, norms_persuasion_model$N_interventions),
  "N (Subjects)" = c(n_total, norms_n, nudge_n, persuasion_n, norms_pers_n),
  "Glass's $\\Delta$ (SE)" = c(mr(model), mr(norms_model), mr(nudge_model), mr(persuasion_model), mr(norms_persuasion_model))
) |> meta_table_maker(
...
table_two <- tibble(
  ...
  "Glass's $\\Delta$ (SE)" = c(mr(health_model), 
    ...
    caption = "Three approaches to MAP reduction persuasion",
    label = "tab:table_two",
        footnote = F) |>
  add_footnote("Note that because many studies present more than one category of message, the number of studies and interventions will sum  to more than the total number of persuasion studies and interventions.", notation = 'none')
table_two
```
...
Second, the `r pap_model$N_studies` studies with a pre-analysis plan have a marginally smaller effect: 
$\Delta$ = `r pap_model$Delta` (SE = `r pap_model$se`), p = `r pap_model$pval`. This difference is not statistically significant.
...
All effect size conversions were conducted by the first author using methods and R code developed for papers [@paluck2019; @porat2024].
...
Second, we applaud researchers and advocates in this field for publicizing null results when they find them.
Third, we notice that the universe of possible interventions is much broader than those we analyzed, suggesting that some promising approaches have yet to be rigorously evaluated (e.g. direct contact with animals on an animal sanctuary, price gradations, high-intensity vegan meal planning, or door-to-door canvassing).
We also note that many viable study sites are absent from our database, e.g. retirement homes, and we encourage researchers to search more broadly for potential subjects.
