...
Studies in our database pursued one or more of three main theories of change: nudges, norms, and persuasion, or a combination of norms and persuasion.
...
This small effect may come as a surprise to readers of previous reviews, which typically found more positive results [@mathur2021meta; @meier2022; @chang2023].
...
However, many studies present a variety of wide variety of outcomes and lead with, e.g., a gender-moderated effect [@piester2020] or the relationship between an underlying attitude measure and consumption outcomes [@berndsen2005].
Others presented meat reduction as one of many components of a broader program of behavior change [@abrahamse2007].
We also find our methods of calculating effect sizes sometimes differ from those presented in papers, and using our estimates of $\Delta$ and standard error, `r nrow(significant_CIs)` interventions have 95% confidence intervals that do not overlap with zero, `r sum(significant_CIs$direction)` of which are positive effects, out of `r nrow(dat)` interventions.

We find some evidence of publication bias in our dataset. $\Delta$ and standard error are positively correlated, though not significantly. Further as we see in table 3, the pooled effect size of interventions published in peer-reviewed journals is about 9 times larger than the equivalent effect size in student theses, while interventions published by advocacy organizations tend to produce a small backlash effect on average.

```{r pbulication_bias_checks, message=F}
...
"our read is quite different' https://www.pnas.org/doi/10.1073/pnas.2200732119
automatacity is not the way forward
